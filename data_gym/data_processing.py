...
def get_home_direction(pos_x,pos_y):
    home_direction_global = np.array([-pos_x, -pos_y])
    transformation_matrix = np.array([[0, -1], [1, 0]])
    home_direction_local = np.matmul(transformation_matrix, home_direction_global)
    home_direction_local = home_direction_local / np.linalg.norm(home_direction_local)

    return home_direction_local
...
with open("./data_gym/processed.csv", "a") as f:
    ...
    for i in range(1,12):
        for j in range(1,12):
            ...
            for k in range(1, 361):
                ...
                gaze = k
                label = get_relative_home_direction(home_direction, gaze)
